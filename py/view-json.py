...
def matches_filter(record: dict, field: str, op: str, value: str,
    ...
    if op == '=':
        if isinstance(value, str) and any(c in value for c in '*?['):
            return fnmatch.fnmatchcase(str(actual), value)
        return str(actual) == str(value)
    elif op == '!=':
        if isinstance(value, str) and any(c in value for c in '*?['):
            return not fnmatch.fnmatchcase(str(actual), value)
        return str(actual) != str(value)
    elif op == '~=':
...
def extract_fields(record: dict, fields: list[str] | None, timestamp_fields: set[str],
                   date_fmt: str) -> dict | None:
    """Extract specified fields from a record. If fields is None, extract all.
    Returns None if any requested field does not exist."""
    if fields is None:
        # Show all fields
        result = {}
    ...
    for field in fields:
        ...
        if value is None:
            return None  # Field doesn't exist, filter out this record
        if field in timestamp_fields and isinstance(value, (int, float)):
            value = convert_timestamp(value, date_fmt)
        result[field] = value
    return result
...
Examples:
  ...
  %(prog)s a.json b.json c.json -f id name
  %(prog)s a.json b.json --no-file-label -f id name
        """
    )
    parser.add_argument('files', nargs='+', help='Input JSON or JSONL file(s)')
    ...
    parser.add_argument('-l', '--filter', action='append', dest='filters', default=None,
                        help='Filter records (e.g., status=active, name=a*b, name~=john, age>=18). '
                             '= and != support bash-style wildcards (*, ?, [..]). If omitted, shows all records.')
    ...
    parser.add_argument('--no-file-label', action='store_true',
    ...
    for filepath in args.files:
        ...
        results = []
        for r in filtered:
            extracted = extract_fields(r, args.fields, timestamp_fields, args.date_format)
